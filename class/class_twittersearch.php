...
private function getKeywords($campaign_id) {
    $query = "SELECT hashtag FROM cs_hashtags WHERE campaign_id = '".$campaign_id."'";
    $result = mysql_query($query) or die("Error: 8 - a.php: ".mysql_error());
  ...
  public function connect() {
      ...
      $temp = array(
                    'q'     =>  $this->getKeywords(1), // query data. if you want to add more than 1, write as "love OR hate", or only hashtag write "#love". check other options at https://dev.twitter.com/docs/using-search
...
//                    'geocode'     => $this->getGeoCode(),       // "latitude,longitude,radius" example: "37.781157,-122.398720,1km"
      ...
      if(curl_errno($ch)) echo "Error: ".curl_errno($ch)." - ".curl_error($ch);
    ...
    $query = "INSERT INTO cs_tweets (
              ...
              VALUES (
                  ...
                  $query .= "'1',
                    ...
                    '".$tweet->created_at."',
                    '".date('Y-m-j H:i:s ', strtotime($tweet->created_at))."',
                    '".$is_active."'
                    )";
...
if($tweets->results)
{
  foreach ($tweets->results AS $tweet) {
    if(!$db->checkTweetAvailable($tweet->id))
    {
      ...
      {
        echo $tweet->id.$tweet->created_at." - ".date('Y-m-j H:i:s ', strtotime($tweet->created_at))." - ".$tweet->text." tweet eklendi<br>\n";     
      }
      else
      {
        echo $tweet->id.$tweet->created_at." - ".date('Y-m-j H:i:s ', strtotime($tweet->created_at))." tweet eklenemedi<br>\n";
      }
    }
    else
    {
      echo $tweet->id.$tweet->created_at." - ".date('Y-m-j H:i:s ', strtotime($tweet->created_at))." bu tweet var<br>\n";
    }
  }
} 
